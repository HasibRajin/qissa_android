...
class SearchRepositories @Inject constructor(
    @ApplicationContext private val context: Context,
    private val api: ApiInterface
) {
    suspend fun callSearchApi(requestData: String, likerId: Int?): SearchResponse {
        return withContext(Dispatchers.IO) {
            SafeApiRequest.apiRequest(context) {
                api.getSearchData(requestData, likerId)
            }
        }
    }
}
